...
- add the shaded regions to the journal prevalence plot 
- remove/refactor plotting of summary data in plots? the real data will be planned-missing
...
illustrate_intercept_prior(mean_prob = .50, sd_logodds = 1.45) # all other modes, given our very weak beliefs and the potential for variation between subfields/journals 

plogis(-2.2)

illustrate_intercept_prior(mean_prob = .25, sd_logodds = 1) 
illustrate_intercept_prior(mean_prob = .25, sd_logodds = 1.2)
illustrate_intercept_prior(mean_prob = .25, sd_logodds = 1.4) 
illustrate_intercept_prior(mean_prob = .25, sd_logodds = 1.6)

illustrate_intercept_prior(mean_prob = .1, sd_logodds = 1) 
illustrate_intercept_prior(mean_prob = .1, sd_logodds = 1.2)
illustrate_intercept_prior(mean_prob = .1, sd_logodds = 1.4) 
illustrate_intercept_prior(mean_prob = .1, sd_logodds = 1.6)
...
# # it is very plausible that sampling at the journal level is so thin that estimating random slopes for each journal isn't feasible and causes convergence issues. In which case we'll drop the random slope at the journal level:
# formulas <- 
#   bf(formula = directbehavioral | weights(percent_of_all_articles) ~               1 + year_centered + (1 + year_centered | p | subfield) + (1 | q | subfield:journal)) +
#   bf(formula = behavioralproxy | weights(percent_of_all_articles) ~                1 + year_centered + (1 + year_centered | p | subfield) + (1 | q | subfield:journal)) +
#   bf(formula = selfreportsaboutbehavior | weights(percent_of_all_articles) ~       1 + year_centered + (1 + year_centered | p | subfield) + (1 | q | subfield:journal)) +
#   bf(formula = selfreportaboutsubjectivestate | weights(percent_of_all_articles) ~ 1 + year_centered + (1 + year_centered | p | subfield) + (1 | q | subfield:journal)) +
#   bf(formula = neurobiopsychophys | weights(percent_of_all_articles) ~             1 + year_centered + (1 + year_centered | p | subfield) + (1 | q | subfield:journal)) +
#   bf(formula = cognitiveability | weights(percent_of_all_articles) ~               1 + year_centered + (1 + year_centered | p | subfield) + (1 | q | subfield:journal))

# # if substantial convergence issues are still encountered, we will switch from modelling all the DVs in a single multivariate model to using separate univariate models for each DV
...
temp <- posterior_prevalence |>
  ungroup() |>
  filter(outcome %in% c("Direct Behavioral", "Self-Report about Subjective States", "Neuro/Bio/Psychophys", "Cognitive Ability")) |>
  mutate(outcome = case_when(outcome == "Direct Behavioral" ~ "Behavior",
                             outcome == "Self-Report about Subjective States" ~ "Subjective experience",
                             outcome == "Neuro/Bio/Psychophys" ~ "Neuro/Bio/Physiological",
                             outcome == "Cognitive Ability" ~ "Cognitive Ability"))


p_prevalence_field <- temp |>
  filter(re_level != "journals") |>
  filter(label == "Psychology (marginal)") |>
  ...
  geom_rect(aes(xmin = 0.50, xmax = 1.00, ymin = -Inf, ymax = Inf, color = NULL),
            fill = "grey95") +
  annotate("text", x = 0.0250, y = "Neuro/Bio/Physiological", label = "Rare",       size = 4, color = "grey30", angle = 90) +
  annotate("text", x = 0.0725, y = "Neuro/Bio/Physiological", label = "Uncommon",   size = 4, color = "grey40", angle = 90) +
  annotate("text", x = 0.1750, y = "Neuro/Bio/Physiological", label = "Occasional", size = 4, color = "grey50", angle = 90) +
  annotate("text", x = 0.3750, y = "Neuro/Bio/Physiological", label = "Common",     size = 4, color = "grey60", angle = 90) +
  annotate("text", x = 0.7500, y = "Neuro/Bio/Physiological", label = "Frequent",   size = 4, color = "grey65", angle = 90) +
...
# posterior_prevalence_field_marginal_by_year_logit <-
#   posterior_prevalence_field_marginal_by_year |>
#   mutate(
#     logit_estimate = qlogis(prevalence_estimate),
#     logit_lower    = qlogis(prevalence_ci_lower),
#     logit_upper    = qlogis(prevalence_ci_upper)
#   )
# 
# p_prevalence_and_trend_field_marginal <- ggplot(posterior_prevalence_field_marginal_by_year_logit,
#        aes(year, logit_estimate)) +
#   geom_ribbon(aes(ymin = logit_lower, ymax = logit_upper), alpha = 0.5) +
#   geom_line() +
#   #scale_x_continuous(breaks = c(2010, 2012, 2014, 2016, 2018, 2020, 2022)) +
#   #coord_cartesian(ylim = c(0,1)) +
#   scale_y_continuous(labels = scales::label_percent()) + # , breaks = c(0, 0.05, 0.1, 0.25, 0.5, 1)
...
posterior_prevalence_field_marginal_by_year_prob <-
  posterior_prevalence_field_marginal_by_year_logit |>
  dplyr::mutate(
    prevalence_estimate  = plogis(logit_estimate),
    prevalence_ci_lower  = plogis(logit_lower),
    prevalence_ci_upper  = plogis(logit_upper)
  )

p_prevalence_and_trend_field_marginal <-
  ggplot(posterior_prevalence_field_marginal_by_year_prob,
         aes(x = year, y = prevalence_estimate)) +
  geom_ribbon(aes(ymin = prevalence_ci_lower,
                  ymax = prevalence_ci_upper),
              alpha = 0.5) +
  geom_line() +
  scale_x_continuous(breaks = c(2010, 2012, 2014, 2016, 2018, 2020, 2022)) +
  coord_cartesian(ylim = c(0, 1)) +
...
temp <- posterior_prevalence_field_marginal_by_year_prob |>
  filter(outcome %in% c("Direct Behavioral", "Self-Report about Subjective States", "Neuro/Bio/Psychophys", "Cognitive Ability")) |>
  mutate(outcome = case_when(outcome == "Direct Behavioral" ~ "Behavior",
                             outcome == "Self-Report about Subjective States" ~ "Subjective experience",
                             outcome == "Neuro/Bio/Psychophys" ~ "Neuro/Bio/Physiological",
...
ggplot(temp,
       aes(x = year, y = prevalence_estimate, fill = outcome)) +
  geom_ribbon(aes(ymin = prevalence_ci_lower,
                  ymax = prevalence_ci_upper),
              alpha = 0.2) +
