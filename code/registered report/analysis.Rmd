...
- correlations among the random effects, not the residual variance terms, which we don't have here. dont misdescribe these as residuals
- could also extract correlations between slopes and intercepts. this would tell us things like "journals that started low tended to increase" or vice versa. 0 correlation: where they start tells you nothing about where they go. watch out for floor/ceiling effects. 
- priors

- check out {marginaleffects} too
- or manual

fitted(newdata = ...,
re = , # on log odds scale. gives you summaries, but not HDIs vs. percentile intervals.
summary = FALSE, # if you want HDIs, use this to not give summaries, instead give an array. compute median and HDIs by column.
robust = TRUE)  # median and MADs.

similar tidybayes functions - drop back to tidybayes if emmeans isn't working? ****

- ensure consistency of language - percentile based intervals vs HDIs.
- HDIs are not invariant under transformation, percentile based intervals are.

formula = outcome ~ 1 + year_centered + (1 + year_centered | p | subfield) + (1 + year_centered | q | subfield:journal)
...
# illustrate that >|4| means always/never
df <- tibble(p = seq(0, 1, by = 0.01)) |>
  mutate(logodds = qlogis(p))

ggplot(df, aes(p, logodds)) +
  geom_point() +
  geom_vline(xintercept = 0.05)

# mildly regularising prior needed to avoid probabilities of (+/-)Inf 

# normal(0, 1) prior would be useful here in general.
...
# intercepts normal(X, 1)
# think about where intercept actually likely to lie for the mean (X): 
# self reports could be 90%, what's the mean for the prior on the log odds scale?:
qlogis(.9)

# slopes for year_centered
# single digit year changes per year? probably agnostic to direction, so mean of 0.
# sd of normal = 0.30 allows for roughly twice what we see below. So SD = .2 to 1 would still be weakly regularising.

# self reports, p for middle year is .9
df2 <- tibble(year_centered = seq(-7, +7, by = 1)) |>
  expand_grid(b1 = seq(-.30, +.30, by = 0.01)) |>
  mutate(logodds = qlogis(.90) + (b1 * year_centered),
         p = plogis(logodds))

ggplot(df2, aes(year_centered, logodds, color = b1, group = b1)) +
  geom_line()

ggplot(df2, aes(year_centered, p, color = b1, group = b1)) +
  geom_line() +
  ylim(0, 1)

# for the random effects, weakly regularizing exp(1)

df3 <- tibble(sd = seq(0, 4, by = 0.1)) |>
  mutate(density = dexp(sd, rate = 1)) # 1 - the point of this would be to rule out values of 10 or 100. still weakly regularising. McElreath uses this frequently in the 2nd edition of rethinking

ggplot(df3, aes(sd, density)) +
  geom_area() +
  xlab("SD (on the beta1 etc parameters on the log odds scale)")
...
scale <- 1  # Scale (spread) parameter

# Create the plot
ggplot(data.frame(x = c(-10, 10)), aes(x = x)) +
  stat_function(fun = function(x) dt((x - location) / scale, df) / scale, 
                color = "blue", size = 1) +
  labs(title = "Student's t-distribution (df = 3, mean = 0, scale = 2.5)",
       x = "X values",
       y = "Density") +
  theme_minimal() +
  xlim(0, NA)
...
# priors on correlations among random effects
# lkj prior, brms default is 1.
# depends on number of correlations. 21 correlations means values of lkj above 1 would be more aggressive. So lkj = 1 is merely skeptical of large correlations. Note that lkj prior is placed on the whole matrix, so even if some correlations are really large its very unlikely all would be large.
# 1 parameter, eta, ranging from 0 to Inf. eta values greater than 1 regularize correlations towards 0. 
# eta of 1 is flat. from 1 to 2 is more like a hill around 0. 
...
if(exists("models/posterior_prevalence.rds")){
  ...
  posterior_prevalence <- 
    bind_rows(posterior_prevalence_field,
              posterior_prevalence_subfields,
              posterior_prevalence_journals) |>
...
\TODO each level has identical estimates, so the random effect is not being taken into account

\TODO add detectable trend from below back here so that color of points change or sth
...
if(exists("models/posterior_trends.rds")){
  ...
  # combine
  posterior_trends <- 
    bind_rows(posterior_trend_field,
              posterior_trend_subfields,
              posterior_trend_journals) |>
...
if(exists("models/posterior_prevalence_field_year.rds")){
  posterior_prevalence_field_year <- read_rds("models/posterior_prevalence_field_year.rds")
} else {
  ...
  posterior_prevalence_field_year <- 
    expand_grid(outcome = outcome_measures,
...
p_prevalence_and_trend_field <- ggplot() +
  geom_ribbon(data = posterior_prevalence_field_year, aes(year, ymin = prevalence_ci_lower, ymax = prevalence_ci_upper), fill = "skyblue", alpha = 0.5) +
  geom_smooth(data = posterior_prevalence_field_year, aes(year, prevalence_estimate),
...
if(exists("models/posterior_prevalence_subfield_year.rds")){
  posterior_prevalence_subfield_year <- read_rds("models/posterior_prevalence_subfield_year.rds")
} else {
  # adjust the previously used function to fit for specific values of year_centered
  ...
  posterior_prevalence_subfield_year <- 
    expand_grid(outcome = outcome_measures,
                data_centered |>
                  distinct(subfield),
                journal = "hypothetical typical journal",
  ...
  write_rds(posterior_prevalence_subfield_year, "models/posterior_prevalence_subfield_year.rds")
}

# plot
p_prevalence_and_trend_subfields <- ggplot() +
  geom_ribbon(data = posterior_prevalence_subfield_year, aes(year, ymin = prevalence_ci_lower, ymax = prevalence_ci_upper), fill = "skyblue", alpha = 0.5) +
  geom_smooth(data = posterior_prevalence_subfield_year, aes(year, prevalence_estimate),
              method = "lm", color = "black", size = 0.75, se = FALSE) +
...
# Combined table with summary conclusions \TODO
...
rnd <- function(x, digits = 2) {
  ...
  # Format the number as a string, retaining trailing zeros
...
posterior_prevalence_and_trends <- 
  full_join(posterior_prevalence, 
            posterior_trends,
            by = c("outcome", "label", "re_level", "subfield", "journal")) |>
  mutate(prevalence_category = case_when(prevalence_estimate < .05 ~ "Rare",
         ...
         trend_category = case_when(trend_ci_lower > 0 ~ "Increasing",
                                    trend_ci_upper < 0 ~ "Decreasing",
                                    TRUE ~ "No trend detected"),
         combined_category = paste(prevalence_category, "and", trend_category),
