...
- elaborate the logic of the priors
- think about model diagnostics more
- Extracted trends are currently identical across levels of the random effect, which can't be right. Fix existing code or write a manual version that estimates trend from multiple estimated means? drop back to tidybayes if emmeans isn't working, or try {marginaleffects}
- table of correlations among random effects could star the estimates with intervals that exclude 0. some commented out code attempting this.
...
- don't misdescribe percentile based CIs with HDIs. below uses percentile CIs
...
# wrangle the correlations at the subfield level of the random effect
...
var_cors_field_intercept |>
  kable(caption = "Correlations among random effects for prevalence at the subfield level") |>
  kable_classic(full_width = FALSE)
